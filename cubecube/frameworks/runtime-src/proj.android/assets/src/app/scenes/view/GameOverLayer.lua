...
function GameOverLayer:ctor(params)
	...
	self.lbl_title = cc.Label:createWithTTF("放不下啦!",Res.font_pingf_bold,56*f_scale,cc.size(0, 0),cc.TEXT_ALIGNMENT_LEFT,cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
							:align(display.CENTER, self.f_width/2, self.f_height-50*f_scale)
				   		    :addTo(self)
	self.lbl_score = cc.Label:createWithTTF("您的分数:",Res.font_pingf_bold,34*f_scale,cc.size(0, 0),cc.TEXT_ALIGNMENT_LEFT,cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
	...
	self.lbl_submit = cc.Label:createWithTTF("重新开始",Res.font_pingf_bold,34*f_scale,cc.size(0, 0),cc.TEXT_ALIGNMENT_CENTER,cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
